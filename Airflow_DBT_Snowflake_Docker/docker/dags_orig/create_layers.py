...
run_dbt_task = DockerOperator(
    ...
    mounts=[Mount(source='./AIRFLOW_DBT_SNOWFLAKE_DOCKER/dbtlearn',target='/dbtlearn',type='bind')],
...
run_dbt_task_4 = DockerOperator(
    ...
    mounts=[Mount(source='./AIRFLOW_DBT_SNOWFLAKE_DOCKER/dbtlearnn',target='/dbtlearn',type='bind')],
    network_mode='container:dbt',  
    dag=dag
)
